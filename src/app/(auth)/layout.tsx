  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <header className="relative z-10 w-full max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
        <BrandLogo variant="full" size="md" subtitle="Dealership Portal Login" />
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          >
            Need an account? Request Demo &rarr;
          </Link>
        </div>
      </header>
      ...
      <footer className="relative z-10 w-full max-w-7xl mx-auto px-6 py-6 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-400 border-t border-slate-900 gap-2">
        <div className="flex items-center gap-2">
          ...
          <span>Enterprise Multi-Tenant Isolated & RBAC Protected</span>
        </div>
        <div>
          &copy; {new Date().getFullYear()} AutoAIdealership (autoaidealership.com). All rights reserved.
        </div>
      </footer>
    </div>
  );
